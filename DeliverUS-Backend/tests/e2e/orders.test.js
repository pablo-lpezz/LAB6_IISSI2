...
describe('Create order', () => {
  ...
  it('Should return 422 when invalid order data - missing address', async () => {
    const invalidOrder = await getNewOrderData(restaurant)
  ...
  it('Should return 422 when invalid order data - invalid product id', async () => {
    const invalidOrder = await getNewOrderData(restaurant)
...
describe('Edit order', () => {
  ...
  it('Should return 409 when order is confirmed', async () => {
    const orderToConfirm = await getNewOrderData(restaurant)
    const confirmedOrder = await createOrder(customer, restaurant, orderToConfirm)
    // Confirm the order so it's no longer pending
    await request(app).patch(`/orders/${confirmedOrder.id}/confirm`).set('Authorization', `Bearer ${owner.token}`).send()
  ...
  it('Should return 409 when order is sent', async () => {
    const orderToSend = await getNewOrderData(restaurant)
    const sentOrder = await createOrder(customer, restaurant, orderToSend)
    // Confirm the order first
    await request(app).patch(`/orders/${sentOrder.id}/confirm`).set('Authorization', `Bearer ${owner.token}`).send()
    // Send the order
    await request(app).patch(`/orders/${sentOrder.id}/send`).set('Authorization', `Bearer ${owner.token}`).send()
  ...
  it('Should return 409 when order is delivered', async () => {
    const orderToDeliver = await getNewOrderData(restaurant)
    const deliveredOrder = await createOrder(customer, restaurant, orderToDeliver)
    // Confirm the order first
    await request(app).patch(`/orders/${deliveredOrder.id}/confirm`).set('Authorization', `Bearer ${owner.token}`).send()
    // Send the order
    await request(app).patch(`/orders/${deliveredOrder.id}/send`).set('Authorization', `Bearer ${owner.token}`).send()
    // Deliver the order
    await request(app).patch(`/orders/${deliveredOrder.id}/deliver`).set('Authorization', `Bearer ${owner.token}`).send()
...
describe('Confirm order', () => {
  ...
  it('Should return 409 when trying to confirm a not pending order', async () => {
    const orderData = await getNewOrderData(await getFirstRestaurantOfOwner(owner))
    const notPendingOrder = await createOrder(customer, await getFirstRestaurantOfOwner(owner), orderData)
    // Confirm the order so it's no longer pending
    await request(app).patch(`/orders/${notPendingOrder.id}/confirm`).set('Authorization', `Bearer ${owner.token}`).send()
...
describe('Send order', () => {
  ...
  it('Should return 409 when trying to send a sent order', async () => {
    const orderData = await getNewOrderData(await getFirstRestaurantOfOwner(owner))
    const sentOrder = await createOrder(customer, await getFirstRestaurantOfOwner(owner), orderData)
    // Confirm the order first
    await request(app).patch(`/orders/${sentOrder.id}/confirm`).set('Authorization', `Bearer ${owner.token}`).send()
    // Send the order
    await request(app).patch(`/orders/${sentOrder.id}/send`).set('Authorization', `Bearer ${owner.token}`).send()
  ...
  it('Should return 200 when valid order', async () => {
    const orderData = await getNewOrderData(await getFirstRestaurantOfOwner(owner))
    order = await createOrder(customer, await getFirstRestaurantOfOwner(owner), orderData)
    // Confirm the order first
...
describe('Deliver order', () => {
  ...
  it('Should return 409 when trying to deliver a confirmed order', async () => {
    const orderData = await getNewOrderData(await getFirstRestaurantOfOwner(owner))
    const confirmedOrder = await createOrder(customer, await getFirstRestaurantOfOwner(owner), orderData)
    // Confirm the order
    await request(app).patch(`/orders/${confirmedOrder.id}/confirm`).set('Authorization', `Bearer ${owner.token}`).send()
  ...
  it('Should return 409 when trying to deliver a delivered order', async () => {
    const orderData = await getNewOrderData(await getFirstRestaurantOfOwner(owner))
    const deliveredOrder = await createOrder(customer, await getFirstRestaurantOfOwner(owner), orderData)
    // Confirm the order first
    await request(app).patch(`/orders/${deliveredOrder.id}/confirm`).set('Authorization', `Bearer ${owner.token}`).send()
    // Send the order
    await request(app).patch(`/orders/${deliveredOrder.id}/send`).set('Authorization', `Bearer ${owner.token}`).send()
  ...
  it('Should return 200 when valid order', async () => {
    ...
    await request(app).patch(`/orders/${order.id}/confirm`).set('Authorization', `Bearer ${owner.token}`).send()
    // Send the order
    await request(app).patch(`/orders/${order.id}/send`).set('Authorization', `Bearer ${owner.token}`).send()
    const response = await request(app).patch(`/orders/${order.id}/deliver`).set('Authorization', `Bearer ${owner.token}`).send()
    expect(response.status).toBe(200)
  })
  it('Should return 200 when requesting order details', async () => {
