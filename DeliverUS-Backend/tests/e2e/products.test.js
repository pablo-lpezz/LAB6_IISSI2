...
describe('Popular products', () => {
  ...
  beforeAll(async () => {
    ...
    const order1 = await createOrder(customer, createdRestaurant, orderData1)
    const order2 = await createOrder(customer2, createdRestaurant2, orderData2)
    // Confirm and send orders to update their status
    await request(await getApp()).patch(`/orders/${order1.id}/confirm`).set('Authorization', `Bearer ${owner.token}`).send()
    await request(await getApp()).patch(`/orders/${order1.id}/send`).set('Authorization', `Bearer ${owner.token}`).send()
    await request(await getApp()).patch(`/orders/${order2.id}/confirm`).set('Authorization', `Bearer ${owner2.token}`).send()
    await request(await getApp()).patch(`/orders/${order2.id}/send`).set('Authorization', `Bearer ${owner2.token}`).send()
  })
